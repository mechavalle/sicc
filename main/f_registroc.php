...
if(isset($_POST['accion']))	
	...
	if($accion==1)
		...
		if(Existe($tabla,"id",$id)=="NE")
			{
			$csql = "INSERT INTO `$tabla` (`descripcion`,`status`,`color`,`ultactfec`,`ultactusu` )";
			$csql .="VALUES ('$descripcion','$status','$color',NOW(),'$IDUser');";
			}
		else{
			$csql = "UPDATE `$tabla` SET `descripcion`='$descripcion', `status`='$status',`color`='$color',`ultactfec`=NOW(),`ultactusu`='$IDUser' ";
			$csql .="WHERE `id`='$id';";
			}
...
<form method="POST" name="edicion" target="_self">
	<div class="container-fluid">
		...
		<table border="0" width="100%" id="table2" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td width="5" height="25">&nbsp;</td>
				<td height="25">
				<input type="color" name="color" size="42" class="cenboxfrmmin" <?echo "value='$color'";?>></td>
			</tr>
		</table>
		<br>
	...
	<input type="hidden" name="minlev" <?echo "value='$minlev'";?>>
	<input type="hidden" name="maxlev" <?echo "value='$maxlev'";?>>
