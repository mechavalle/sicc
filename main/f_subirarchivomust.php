...
if(isset($_POST['accion']))	
	...
	if($accion==1)
		...
		if(strlen(basename($_FILES['foto']['name']))>0)
			...
			$nuevonombre=completa($id,"0","9","S")."_".MD5($dir.Date("Y-m-d h:i:s")).".".$extension;
			$uploadfile = $dir."/".$nuevonombre;
			...
			if(subearchivo("foto",$uploadfile)=="OK")
				{
					...
					#echo "<br>3";			
					$csql = "UPDATE `$tabla` SET archivo='$nuevonombre',tipoarchivo='".$_FILES['foto']['type']."',tamaarchivo='".$_FILES['foto']['size']."',fecha='".date("Y-m-d h:i:s")."',ultactfec='".date("Y-m-d h:i:s")."',ultactusu='$IDUser' WHERE id='$id'";
					mysqli_query($conexio,$csql);
					if(mysqli_error($conexio)!=""){
						echo "Error al actualizar la imagen: ".mysqli_error($conexio)."->$csql";
						exit();}
...
function go()
	...
	if(document.edicion.foto.value=="")
		{
		alert ("seleccione algún documento para continuar");
		return "";	
		}
