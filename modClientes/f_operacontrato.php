...
$consulta="Select * from adm_documentos where identificador='$idcontrato'";
...
if(isset($_POST['accion']))	
	...
	if($accion==1)
		...
		$fecha=asigna("fecha");
		$secretario=asigna("secretario");
		$servicio=asigna("servicio");
		$honorarios=asigna("honorarios");
		$fechadoc=asigna("fechadoc");
		$lugar=asigna("lugar");
		
		if(Existe("ope_contratos","id",$id)=="NE")
			{
			$csql = "INSERT INTO `ope_contratos` (`idpersona`,`idcontrato`,`descripcion`,`puesto`,`departamento`,`actividades`,";
			$csql .="`honorarios`,`servicio`,`secretario`,`duracion`,`sueldodiario`,`jornada`,`fecha`,`fechadoc`,`lugar`,`ultactfec`,`ultactusu` )";
			$csql .="VALUES ('$idpersona','$idcontrato','$descripcion','$puesto','$departamento','$actividades',";
			$csql .="'$honorarios','$servicio','$secretario','$duracion','$sueldodiario','$jornada','$fecha','$fechadoc','$lugar',NOW(),'$IDUser');";
			}
		else
			{
			$csql = "UPDATE `ope_contratos` SET `idcontrato`='$idcontrato', `descripcion`='$descripcion', `puesto`='$puesto',`duracion`='$duracion',";
			...
			$csql .="`honorarios`='$honorarios',`servicio`='$servicio',`secretario`='$secretario',`fechadoc`='$fechadoc',`ultactusu`='$IDUser' ";
			$csql .="WHERE `id`='$id';";
			}
		mysqli_query($conexio, $csql);
		if(mysqli_error($conexio)!="") {
			echo "ERROR: "."Error al grabar el registro. ".mysqli_error($conexio);
			exit(); }
		else
			{
			if($id=="-1")
	...
	if($accion==5)
		{		
		$csql="delete from ope_contratos where id='$id'";
		mysqli_query($conexio, $csql);
...
function eliminar()
	{
	if(confirm('¿Desea eliminar permanantemente este contrato?'))
		{
...
<div class="container-fluid">
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
		<tr>
			...
			<td width="100" align="right"><div id="divclose"><a href="#" onclick="window.close();"><span class="glyphicon glyphicon-remove-circle" style="font-size: 30px; float: right;"></span></a></div></td>
		</tr>
	</table>	
	</div>
...
<?
if($id>0)
	echo "<button type='button' onClick=\"eliminar();\" class='btn btn-danger btn-xs'>Eliminar Documento</button>";
?>
...
<table border="0" width="90%" id="table3" cellspacing="0" cellpadding="0">
	<tr>
		<td width="147" align="right" height="25" bordercolor="#FFFF99">
		<font face="Arial" size="2">Fecha Inicio Labores:</font></td>
		<td width="10" align="left" height="25" bordercolor="#FFFF99">
		&nbsp;</td>
		<td align="left" height="25" bordercolor="#FFFF99">
		<input type="text" name="fecha" size="13" style="font-family: Verdana; font-size: 8pt" <?echo "value='$fecha'"?>><a href="#" name="anchor2" id="anchor2" onClick="displayCalendar(document.forms[0].fecha,'yyyy-mm-dd',this);"><img border="0" src="../img/calendar.gif" width="16" height="15"></a> </td>
	...
	<tr>
		<td width="147" align="right" height="25" bordercolor="#FFFF99">
		<font face="Arial" size="2">Fecha Documento:</font></td>
		<td width="10" align="left" height="25" bordercolor="#FFFF99">
		&nbsp;</td>
		<td align="left" height="25" bordercolor="#FFFF99">
...
<input type="hidden" name="idcontrato" <?echo "value='$idcontrato'";?>>
